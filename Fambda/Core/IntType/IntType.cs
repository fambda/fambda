...
namespace Fambda
{
    ...
    public static class IntType
    {
        ...
        public static Option<int> Parse(string s)
            => int.TryParse(s, out var parseResult) ? Some(parseResult) : None;
        ...
        /// <returns><see cref="Option{T}"/> with <see cref="OptionSome{T}"/> if <paramref name="s" /> was converted successfully; otherwise, <see cref="OptionNone"/>.</returns>
        [Pure]
        public static Option<int> Parse(string s, NumberStyles style)
        {
            try
            {
                var parseResult = int.Parse(s, style);
        ...
        public static Option<int> Parse(string s, NumberStyles style, IFormatProvider provider)
            => int.TryParse(s, style, provider, out var parseResult) ? Some(parseResult) : None;
    }
}
