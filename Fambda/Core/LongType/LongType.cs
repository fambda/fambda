...
namespace Fambda
{
    ...
    public static class LongType
    {
        ...
        /// <returns><see cref="Option{T}"/> with <see cref="OptionSome{T}"/> if <paramref name="s" /> was converted successfully; otherwise, <see cref="OptionNone"/></returns>
        [Pure]
        public static Option<long> Parse(string s, NumberStyles style)
        {
            try
            {
                var parseResult = long.Parse(s, style);
                return Some(parseResult);
            }
            catch
            {
        ...
        /// <returns><see cref="Option{T}"/> with <see cref="OptionSome{T}"/> if <paramref name="s" /> was converted successfully; otherwise, <see cref="OptionNone"/></returns>
        [Pure]
        public static Option<long> Parse(string s, IFormatProvider provider)
        {
            try
            {
                var parseResult = long.Parse(s, provider);
                return Some(parseResult);
            }
            catch
            {
